...
This fork replaces the entire native VNC server stack with a pure Rust implementation:
...
For comprehensive technical details about the Rust VNC implementation, including encoding specifications and performance characteristics, see [rustvncserver/TECHNICAL.md](https://github.com/dustinmcafee/rustvncserver/blob/main/TECHNICAL.md).
...
1. Install the app (see [Building](#building) section).
...
This Rust edition requires:

1. **Rust toolchain** (1.76 or newer):
   ```bash
...
# Build full debug APK (automatically builds Rust first)
...
The Rust VNC server library will be compiled for:
- ARM64 (`arm64-v8a`)
- ARMv7 (`armeabi-v7a`)
- x86 (`x86`)
- x86_64 (`x86_64`)
...
This fork maintains compatibility with the original droidVNC-NG at the application level. Contributions are welcome for:

- Rust VNC server improvements
- Performance optimizations
- Bug fixes
- Additional VNC protocol features
...
- **Original droidVNC-NG**: [bk138/droidVNC-NG](https://github.com/bk138/droidVNC-NG) - Christian Beier
- **Rust VNC Implementation**: This fork
- **VNC Protocol**: Based on the RFB protocol specification

## License

This project maintains the same license as the original droidVNC-NG project (GPL-2.0).
